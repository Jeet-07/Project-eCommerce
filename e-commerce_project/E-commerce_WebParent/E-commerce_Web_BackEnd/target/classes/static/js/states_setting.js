...
function deleteState() {
	...
	$.ajax({
		...
		changeFormStateToNew();
		showStatesToastMessage("The state has been deleted");
	}).fail(function() {
		showStatesToastMessage("ERROR: Could not connect to server or server encountered an error");
	});		
}
...
function updateState() {
	...
	$.ajax({
		...
		showStatesToastMessage("The state has been updated");
		changeFormStateToNew();
	}).fail(function() {
		showStatesToastMessage("ERROR: Could not connect to server or server encountered an error");
	});	
...
function addState() {
	...
	$.ajax({
		...
		showStatesToastMessage("The new state has been added");
	}).fail(function() {
		showStatesToastMessage("ERROR: Could not connect to server or server encountered an error");
	});
...
function loadStates4Country() {
	...
	$.get(url, function(responseJSON) {
		...
		changeFormStateToNew();
		showStatesToastMessage("All states have been loaded for country " + selectedCountry.text());
	}).fail(function() {
		showStatesToastMessage("ERROR: Could not connect to server or server encountered an error");
	});	
}
...
function loadCountries4States() {
	...
	$.get(url, function(responseJSON) {
		...
		showStatesToastMessage("All countries have been loaded");
	}).fail(function() {
		showStatesToastMessage("ERROR: Could not connect to server or server encountered an error");
	});
}

function showStatesToastMessage(message) {
	$("#toastStatesMessage").text(message);
	$(".toast-state").toast('show');
}